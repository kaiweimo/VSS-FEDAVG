...
class ML_Dataset(): 
    ...
    def get_datasets(self, dataset_name):
        ...
        if dataset_name == 'EMnist':
            train_dataset = datasets.MNIST(root='./Pytorch/emnist/', train=True, transform=transforms.ToTensor(), download=True)
            test_dataset = datasets.MNIST(root='./Pytorch/emnist/', train=False, transform=transforms.ToTensor())
            class_num = 62
        if dataset_name == 'Cifar10':
            train_dataset = datasets.CIFAR10(root='./Pytorch/cifar10/', train=True, transform=transforms.ToTensor(), download=True)
            test_dataset = datasets.CIFAR10(root='./Pytorch/cifar10/', train=False, transform=transforms.ToTensor())
            class_num = 10
        if dataset_name == 'KWS':
            train_dataset = KWSconstructor(root='./Pytorch/kws/index_train.txt', transform=None)
            test_dataset = KWSconstructor(root='./Pytorch/kws/index_test.txt', transform=None)
            class_num = 10
        if dataset_name == 'ag_news':
            train_dataset = ag_newsconstructor(root='./Pytorch/out/train_index.txt', transform=None)
            test_dataset = ag_newsconstructor(root='./Pytorch/out/test_index.txt', transform=None)
            class_num = 4
